      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative language-menu">
          ...
          >
            ...
            <svg className={`w-4 h-4 ml-1 inline transition-transform ${showLanguageMenu ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path>
            </svg>
          </button>
          {showLanguageMenu && (
            <div className="absolute right-0 mt-2 w-48 border rounded-lg shadow-lg py-2 z-20 bg-white border-gray-200">
              <div className="px-4 py-2 border-b border-gray-100">
                <h3 className="text-sm font-medium text-gray-900">Language</h3>
              </div>
              <div className="py-1">
                <button onClick={() => { setLanguage('en'); setShowLanguageMenu(false); }} className={`flex items-center justify-between w-full px-4 py-2 text-sm transition-colors ${language === 'en' ? 'bg-blue-50 text-blue-600' : 'text-gray-700 hover:bg-gray-50'}`}>English</button>
                <button onClick={() => { setLanguage('ms'); setShowLanguageMenu(false); }} className={`flex items-center justify-between w-full px-4 py-2 text-sm transition-colors ${language === 'ms' ? 'bg-blue-50 text-blue-600' : 'text-gray-700 hover:bg-gray-50'}`}>Bahasa Melayu</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="relative user-menu">
          ...
          {showUserMenu && (
            <div className="absolute right-0 mt-2 w-48 border rounded-lg shadow-lg py-2 z-20 bg-white border-gray-200">
              ...
              <div className="py-1">
                <button onClick={() => { router.push('/seller/profile'); setShowUserMenu(false); }} className="flex items-center gap-3 w-full px-4 py-2 text-sm hover:bg-gray-50 transition-colors text-gray-700">Edit Profile</button>
                <button onClick={() => { router.push('/'); setShowUserMenu(false); }} className="flex items-center gap-3 w-full px-4 py-2 text-sm hover:bg-gray-50 transition-colors text-gray-700">Browse Products</button>
              </div>
              <div className="border-t border-gray-100 pt-1">
                <button onClick={() => { handleLogout(); setShowUserMenu(false); }} className="flex items-center gap-3 w-full px-4 py-2 text-sm text-red-600 hover:bg-red-50"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"></path></svg>Logout</button>
              </div>
            </div>
          )}
        </div>
