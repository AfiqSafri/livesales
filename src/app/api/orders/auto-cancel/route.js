...
export async function POST(req) {
  try {
    ...
    console.log(`Found ${unpaidOrders.length} unpaid orders older than 3 minutes`);
    ...
    for (const order of unpaidOrders) {
      try {
        ...
        if (order.buyerEmail) {
          try {
            console.log(`Sending cancellation email to: ${order.buyerEmail}`);
            ...
            if (emailResponse.ok) {
              console.log(`Cancellation email sent successfully to ${order.buyerEmail}`);
            } else {
              console.error(`Failed to send cancellation email to ${order.buyerEmail}`);
            }
          } catch (emailError) {
            console.error(`Error sending cancellation email to ${order.buyerEmail}:`, emailError);
          }
        } else {
          console.log(`No buyer email found for order #${order.id}`);
        }

        console.log(`Cancelled order #${order.id} and restored ${order.quantity} units to product #${order.productId}`);
      } catch (error) {
